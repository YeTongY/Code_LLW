...
《代号：LLW》是一个 3 人团队开发的 JRPG 游戏,使用 C 风格的 C++ 和 Raylib 图形库。游戏包含地图探索、回合制战斗等核心玩法。
...
对于不影响功能的大量修改（例如拼写修正、注释更新、小范围文档调整或小型格式修复），请使用 `misc` 或 `fix`（如果是修复）类型，但建议统一格式如下：

- 格式: `misc: 简短描述` 或 `fix: 简短描述`（当确实修复了 bug 时）
- 说明: 在提交主体里使用一句简短描述 + 可选的额外行，解释为何需要这次小修改。

示例:

```
misc: 修正 README 中的错别字

修正了“配置”一词的拼写错误，并微调了路径示例，避免新手误操作。
```

或者当是小的 bug 修复:

```
fix: 修复 GameState 头文件重复定义问题

将 `GameEngine` 结构体移动到头文件保护宏内部，避免重复包含导致的重新定义编译错误。
```
